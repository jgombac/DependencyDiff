...
from .text import WordMatcher, split_text
    ...
    if clean:
        ...
        for node in list(walk_dom(dom)):
            if node.nodeName == 'style':
                ...
                unwrap(node)
            elif node.nodeType == Node.TEXT_NODE and node.nodeValue != " ":
                text = dom.createElement('text-node')
                text.setAttribute("text-value", node.nodeValue)
